...
class paginatorTest extends TestCase
{
    ...
    public function testPreviousDisabled()
    {
        ...
        $this->assertStringContainsString('previous', (string)$attrs['class']);
        $this->assertStringContainsString('disabled', (string)$attrs['class']);
    }
    ...
    public function testNextDisabled()
    {
        ...
        $this->assertStringContainsString('next', (string)$attrs['class']);
        $this->assertStringContainsString('disabled', (string)$attrs['class']);
    }
    ...
    public function testRightTrunc()
    {
        ...
        $attrs = $out->li[0]->attributes();
        $this->assertStringContainsString('previous', (string)$attrs['class']);
        $this->assertStringContainsString('enabled', (string)$attrs['class']);

        for ($i = 1; $i <= 6; $i++) {
            $attrs = $out->li[$i]->attributes();
            $this->assertEquals((string)$out->li[$i]->a->span, (string)$i);
            if ($i == 2) {
                $this->assertStringContainsString('current', (string)$attrs['class']);
            } else {
                $this->assertStringNotContainsString('current', (string)$attrs['class']);
            }
        }
